...
self.on('click', function (node, data) {
  function parseSoundcloudState(node) {
    const time = Array.from(document.querySelector('.playbackTimeline__timePassed')
                            .querySelectorAll('span'))
          .filter(el => return el.hasAttribute('aria-hidden')).textContent;
    const volume = document.querySelector('.volume__sliderWrapper').getAttribute('aria-valuenow');
    return {
      time: time,
      volume: volume
    };
  ...
  function findLinkNode(node) {
    if (node && node.href) {
      ...
      if (node.classList.contains('playbackSoundBadge__title')) {
        Object.assign(opts, parseSoundcloudState(node));
      }
      self.postMessage(opts);
    }
